...
### 改进
- **恢复期检查更及时**：网络异常后从固定 10 秒快速轮询调整为 `2s -> 5s -> 10s` 封顶退避；配合已有 `NotifyAddrChange` 事件唤醒，刚断线/刚恢复时更快进入下一次检查，稳定在线后仍回到正常轮询间隔。
- **运行时网络查询更轻**：对物理网卡 IP、默认网关等昂贵 Windows 查询增加短 TTL 缓存，并在系统网络变化事件发生时立即失效，同时清理缓存源 IP，避免 Wi-Fi 切换后沿用旧路由信息。
- **发布一致性校验**：新增 `verify_release.py`，用于校验本地 exe、git tag、`origin/master`、GitHub Release 附件大小和 SHA256 是否一致。

### 测试
- CI 现在同时运行单元测试和集成测试，并检查发布校验脚本语法。
- 新增恢复期退避、网络缓存失效、自启禁用提示等回归测试。

### 兼容性
- 不改变登录、代理绕过、Wi-Fi 重连、开机自启、托盘菜单、配置格式和命令行参数。

## v1.4.4 (2026-07-05)

### 改进
- **启动网络就绪检测更轻**：缓存 Windows 开机时间，避免同一进程内重复调用 PowerShell 查询；`network_ready()` 在非日志轮询阶段只做关键 portal TCP 探测，不再反复查询网卡和默认网关，减少开机/恢复等待循环中的额外开销。
- **exe 体积继续压缩**：托盘图标改为仅用 `PIL.Image` 在内存中生成，不再依赖 `ImageDraw/ImageFont`；PyInstaller 排除对应 Pillow 字体/CMS 模块，在保留托盘功能的前提下降低打包体积。
...
### 改进
- **日志改为单文件自动限容,不再按天累积**。此前每启动一天就生成一个 `campus_auto_login_py_YYYYMMDD.log`,会越积越多。现改为单个 `campus_auto_login_py.log`,超过 256KB 自动归档为一个 `.log.old`(总占用≈0.5MB,永不增长),并在启动时自动清理旧的按天日志文件。排障能力完整保留。

### 测试
- 新增日志清理回归测试,单元+集成共134项全部通过。
