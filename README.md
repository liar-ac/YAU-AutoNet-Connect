...
| 即时掉线检测 (v1.3.0) | 事件驱动监听网卡变化，掉线/重连约 1 秒内感知并处理，无需等待轮询周期 |
| 永不放弃守护 (v1.3.0) | 监控异常多次后转入慢速守护持续重试，不会静默停止 |
...
| 日志增强 (v1.2.0) | 支持日志级别控制（debug/info/warning/error）、单文件自动限容、一键导出故障排查包 |
...
| 日志自动限容 | 单个日志文件，超过 256KB 自动归档为 `.log.old`，不按天累积（总占用≈0.5MB） |
...
程序运行时会自动检测网络状态：在线时跳过，离线时尝试登录。日志为单个文件，超过 256KB 自动归档为 `.log.old`，不会按天累积。
